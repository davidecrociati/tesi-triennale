...
\begin{itemize}
    ...
    \item ribaltamento in orizzontale;
...
\\Tale framework si è dimostrato estremamente flessibile e adatto per lo sviluppo del prototipo in questione.
...
\begin{lstlisting}[language=rust, caption={Porzione del file main.rs}, showstringspaces=false]
#[actix_rt::main]
...
\newpage
\subsection{Modulo WebAssembly/WASI}
...
\end{lstlisting}
Il file è aperto mediante l'impego del metodo \textbf{Jimp.read()}, il quale fornisce un'istanza utilizzabile per modificare l'immagine secondo specifiche.
...
\end{equation*}
Successivamente verranno presentati i grafici per ciascuno dei parametri scelti, utilizzando rispettivamente Rust in combinazione con WebAssembly e Node.js. Ogni grafico conterrà i risultati per ognuna delle tre immagini utilizzate, consentendo così anche una facile analisi visiva.
\subsection{Latenza}
...
\\In particolare, Node.js presenta un consumo di memoria leggermente maggiore, ma in entrambi i casi, tale parametro viene mantenuto pressochè costante per ciascuna tipologia di immagine, non presentando differenze significative.
\newpage
\begin{figure}
    \begin{center}
